    ...
    <script>
      document.getElementById('form').addEventListener(
        ...
        //   const telpRegex = /^\+[0-9]+( [0-9]+)+$/;
          ...
          inputs.forEach(
            (input) => (input.style.borderBottom = '1px solid #CBD5E1')
          );
          notifDiv.innerHTML = '';

          //   cross check
          if (usnInput.value.trim() === '') {
            return kasitahuError(event, usnInput, 'Username wajib diisi!');
          }
          if (passInput.value.trim() === '') {
            return kasitahuError(event, passInput, 'Password wajib diisi!');
          }
          if (namaInput.value.trim() === '') {
            return kasitahuError(event, namaInput, 'Nama Lengkap wajib diisi!');
          }
          if (telpInput.value.trim() === '') {
            return kasitahuError(event, telpInput, 'No Telepon wajib diisi!');
          }
          if (emailInput.value.trim() === '') {
            return kasitahuError(event, emailInput, 'Email wajib diisi!');
          }

        //   if (!telpRegex.test(telpInput.value.trim())) {
        //     return kasitahuError(
        //       event,
        //       telpInput,
        //       'Format No Telp salah! Harus pakai + dan spasi (cth: +62 812 3456)'
        //     );
        //   }
          ...
          function kasitahuError(e, inputElement, pesan) {
            e.preventDefault();
            e.stopImmediatePropagation();
            notifDiv.innerHTML = pesan;
            inputElement.style.borderBottom = '2px solid #EF4444';
            inputElement.focus();
            return false;
          }
        },
      ...
      const semuaInput = ['usn', 'pass', 'nama', 'telp', 'email'];
